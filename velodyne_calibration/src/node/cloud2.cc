...
#define NODE "velodyne_calibrate_cloud2"
...
namespace {
  ...
  sensor_msgs::PointCloud2 cloud2_;           // outgoing PointCloud message
...
void processXYZ(const std::vector<velodyne::laserscan_xyz_t> &scan) {
  ...
  data->getMsgHeaderFields(cloud2_.header.stamp, cloud2_.header.frame_id);

  size_t numPoints = scan.size();
  velodyne_.getPointCloud2(scan, cloud2_);
  //velodyne_.getPointCloud(scan, &cloud_);
  //sensor_msgs::convertPointCloudToPointCloud2(cloud_, cloud2_);

  ROS_DEBUG(NODE ": Publishing %u Velodyne points.", numPoints);
  output.publish(cloud2_);
}
...
int main(int argc, char *argv[]) {
  ...
  output = node.advertise<sensor_msgs::PointCloud2>("velodyne/pointcloud2", qDepth);

  velodyne_.preAllocatePointCloud2(cloud2_);
  //velodyne_.preAllocatePointCloud(&cloud_);
